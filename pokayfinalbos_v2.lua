...
-- Load Library WindUI (parallel pre-fetch agar tidak block)
local WindUI
do
...
local GH_TOKEN        = "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"
...
local ADMIN_NAMES = { ["jrkigacor420"] = true, ["cansu_dilaraa"] = true }
...
-- Cached whitelist entries (agar auto-login tidak fetch ulang)
local _cachedWLEntries = nil
...
local function ValidateToken(token)
    ...
    -- Pakai cache jika sudah ada (skip HTTP untuk auto-login)
    local entries = _cachedWLEntries
    if not entries then
        ...
        local rawJson = ""
        -- Langsung decode base64 tanpa proxy jika executor support
        local okDirect, directRes = pcall(function()
            local decoded = ""
            -- coba decode langsung
            local chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
            b64 = b64:gsub("[^"..chars.."=]","")
            decoded = b64:gsub(".", function(x)
        ...
        if okDirect and directRes and #directRes > 2 then
            rawJson = directRes
        else
            -- fallback download_url langsung (1 request, lebih cepat dari allorigins)
            local okDl, dlRes = pcall(function() return game:HttpGet(ghData.download_url or "") end)
            if okDl and dlRes and #dlRes > 2 then
                rawJson = dlRes
            else
                ...
                if okB and bRes and #bRes > 2 then rawJson = bRes
                else return false, "Gagal decode whitelist" end
            end
        end

        local okP, parsed = pcall(function() return HttpService:JSONDecode(rawJson) end)
        if not okP or type(parsed) ~= "table" then return false, "Format whitelist tidak valid" end
        entries = parsed
        _cachedWLEntries = entries -- simpan cache
    end
...
local function createAndUnlockAllTabs()
	...
	-- Create Logout Tab (PALING BAWAH - dibuat terakhir)
...
local function verifyAndLogin(token)
    ...
    if valid then
        ...
        -- Ambil user info langsung (tidak perlu wait, sudah ada cache _WL_ENTRY)
        ...
        if not menuCreated then
            -- Setup semua tab sekaligus tanpa jeda
            setupAccountTab()
            setupCreditsTab()
            setupBypassTab()
            setupListScript()
            ...
            setupWebhookTab()
            setupCustomNameTab()
            setupLogoutTab() -- Logout PALING BAWAH
            menuCreated = true
        end
...
task.spawn(function()
    ...
    if savedToken and tostring(savedToken) ~= "" and #tostring(savedToken) >= 5 then
        ...
        if valid then
            ...
            -- Ambil user info langsung (cache sudah ada)
            ...
            WindUI:Notify({
                ...
                Content = "Welcome back, " .. RobloxUsername .. "!",
                Duration = 3,
                Icon = "check-check",
            })
        else
